...
const ROUTES: LocalisedRoute[] = [
  ...
  // Commercial service pages, plus the hub that links to both. Italian only
  // by design - see CLAUDE.md - so `locales` is `['it']`, not `LANGUAGES`:
  // the same guard that keeps a locale-restricted blog article from
  // advertising an hreflang alternate that 404s applies here too.
  {
    path: '/servizi',
    locales: ['it'],
